							...
							int nDotPos = line.find(".");
									switch(nDotPos){
										...
										case 0: //dot occurs
										{
											 ...
											 i=tempAddrStr;
											 //cout<<tempAddrStr<<endl;
											 //line="";
											 if(myfile.good()){
												getline (myfile,line);
											 }
											
											 //line.replace(tempAddrStr, 1, " ");
											 goto LOOP1;
     										/*	 nDotPos = line.find(".");
												if(nDotPos ==0){
													goto LOOP;
		...
		//	cout<<ir<<endl;
						...
						switch(ir)
						{
							...
							case 6: // Load the value from  (sp+x) into the AC
									...
									count = ss.str();
									//cout<<"Printing count = "<<count<<" Printing ram - "<<ram[abc]<<endl;
									write(p2c[1], (void*)count.c_str(), 10);									
									read(c2p[0], buf, 10);
